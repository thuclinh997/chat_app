...
if (!empty($fname) && !empty($lname) && !empty($email) && !empty($password)) {
    //let's check user email is valid of not
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) { //if email is valid
        //let's check that email already exists in the database of not
        ...
        if (mysqli_num_rows($sql) > 0) { //if email already exist
            ...
            if (isset($_FILES['image'])) { //if file is uploaded 
                ...
                $extension = ['png', 'jpeg', 'jpg']; //these are valid img ext and we've store them in array
                if (in_array($img_ext, $extension) === true) { //if user uploaded img ext is matched with any array extension
                    $time = time(); //this will return us crrent time..
                    //we need this time because when you uploading user img to in our
                    //folder we rename user file with current time
                    ...
                    if (move_uploaded_file($tmp_name, "images/" . $newImgName)) { //
                        ...
                        $sql2 = mysqli_query($conn, "INSERT INTO users
                                ...
                                VALUES ('{$random_id}','{$fname}','{$lname}', '{$email}', '{$password}','{$newImgName}','{$status}')");
                        if ($sql2) { //if these dada intered
                            $sql3 = mysqli_query($conn, "SELECT * FROM users WHERE email = '{$email}' LIMIT 1");
                            if (mysqli_num_rows($sql3) > 0) {
                                $row = mysqli_fetch_assoc($sql3);
                                $_SESSION['unique_id'] = $row['unique_id']; //using the session we used unique_id in orther php file;
                                echo "success";
                            }
                        } else {
